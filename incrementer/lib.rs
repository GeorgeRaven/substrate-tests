...
mod incrementer {
    ...
    impl Incrementer {
        ...
        fn inc_mine(&mut self, by: u64) {
            let caller = &self.env().caller(); // ACTION: Get the `caller` of this function.
            let my_number = self.my_value_or_zero(caller); // ACTION: Get `my_value` that belongs to `caller` by using `my_value_or_zero`.
            self.my_value.insert(*caller, my_number+by); // ACTION: Insert the incremented `value` back into the mapping.
        }
